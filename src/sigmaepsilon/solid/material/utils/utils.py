...
from sigmaepsilon.math.linalg import is_pos_def
...
def _get_elastic_params(isoplane: str = None, **params):
    ...
    if nParams == 2:
        ...
        raise MaterialError("Invalid material definition!")
...
def elastic_compliance_matrix(
    ...
    Raises
    ------
    ~sigmaepsilon.solid.material.hooke.utils.MaterialError
...
def membrane_elastic_material_stiffness_matrix(
    symbolic: Optional[bool] = False, **kwargs
) -> ndarray:
    """
    Returns the elastic material matrix for a membrane in plane stress state.
    """
    S = smat_sym_ortho_3d()
    S.row_del(2)
    S.row_del(2)
...
def mindlin_elastic_material_stiffness_matrix(
    symbolic: Optional[bool] = False, **kwargs
) -> ndarray:
    """
    Returns the elastic material matrix for Uflyand-Mindlin plates and shells.
    """
    S = smat_sym_ortho_3d()
    S.row_del(2)
    S.col_del(2)
